...
from electrum_mrng.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds
from electrum_mrng import bitcoin
from electrum_mrng.util import timestamp_to_datetime
...
class SendScreen(CScreen):
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a MorningStar address or a payment request'))
                return
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid MorningStar Address') + ':\n' + address)
                return
            try:
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from electrum_mrng.util import create_URI
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share MorningStar Request"))
