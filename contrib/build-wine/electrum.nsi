...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-MRNG"
  !define PRODUCT_WEB_SITE "https://github.com/MorningStarTeam/electrum-mrng"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-mrng-setup.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-mrng-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-mrng-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-mrng-${PRODUCT_VERSION}.exe" 0

  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\morningstar" "" "URL:morningstar Protocol"
  WriteRegStr HKCU "Software\Classes\morningstar" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\morningstar" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\morningstar\shell\open\command" "" "$\"$INSTDIR\electrum-mrng-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\morningstar"
