...
SYSTEM_CONFIG_PATH = "/etc/electrum-mrng.conf"
...
class SimpleConfig(PrintError):
    ...
    def get_wallet_path(self):
        ...
        old_path = os.path.join(self.path, "electrum-mrng.dat")
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/electrum-mrng.conf."""
    result = {}
    if os.path.exists(path):
        try:
            import ConfigParser
        except ImportError:
            print "cannot parse electrum-mrng.conf. please install ConfigParser"
            return
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-mrng.conf into user_config[]."""
    if not path:
        return {}
